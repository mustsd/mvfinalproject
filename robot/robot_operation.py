...
def move_L(move: DobotApiMove, point_list: list):
    move.MovL(point_list[0], point_list[1], point_list[2], point_list[3])

def move_J(move: DobotApiMove, point_list: list):
...
def pick(pose_list: list ):
    
    dashboard, move, feed = ConnectRobot()
    print("start enable robot...")
    dashboard.EnableRobot()
    print("finish enable robot:)")
    ...
    for pose in pose_list:
        ...
        dashboard.DO(1, 1)  # pick
        sleep(0.5)

        # back to approach point
        move_L(move, approach_point)
        WaitArrive(approach_point)
